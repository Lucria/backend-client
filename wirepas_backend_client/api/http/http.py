...
class PerformanceValue():
    ...
    def toString(self):
        ...
        if self.__countableValue is True:
            ret = "[cnt] Interval count:{} Reported values avg:{} " \
            ...
            ret = "[dur] Interval count:{} Reported values interval avg:{:.1f} " \
                  "intervalDuration(s):{:.1f}. Session min value:{:.1f} Session max value:{:.1f}".format(
                self.getIntervalUpdateCount(),
                self.getIntervalAverageValue(),
                self.getIntervalDuration(),
                self.getSessionMinValue(),
